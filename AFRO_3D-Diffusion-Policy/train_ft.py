...
from diffusion_policy_3d.common.addition import plot_history, calculate_average_metrics
...
class TrainDP3Workspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        vis_param_ids = {id(p) for p in self.model.vis_encoder.parameters()}

        # 2. 非 vis 参数
        base_params = [p for p in self.model.parameters()
                    if id(p) not in vis_param_ids]

        # 3. 分别实例化两段优化器（用你 cfg 里写的任意类）
        vis_opt   = hydra.utils.instantiate(cfg.optimizer_vis,
                                            params=self.model.vis_encoder.parameters())
        base_opt  = hydra.utils.instantiate(cfg.optimizer, params=base_params)

        # 4. 把 param_groups 合并到一个优化器里，后续完全透明
        self.optimizer = torch.optim.AdamW([*base_opt.param_groups,
                                            *vis_opt.param_groups])
